...
\begin{column}{0.48\textwidth}
  \textbf{The catch}
  \begin{itemize}
    \item We were told a subset of training labels had been \textit{deliberately flipped}
...
\begin{column}{0.40\textwidth}
  ...
  \begin{itemize}
    ...
    \item Many binary features contribute under 5\% each but collectively matter
  \end{itemize}
  }
\end{column}
...
{\footnotesize
\textbf{TabPFN} (Hollmann et al., 2022) is a transformer \textit{meta-trained} (trained to learn from new tasks) on millions of synthetic tabular classification datasets.
}
...
\begin{column}{0.38\textwidth}
  ...
  \begin{itemize}
    ...
    \item Approximates the Bayesian posterior \mbox{predictive: $P(y_{\text{test}} \mid X_{\text{test}},\, X_{\text{train}},\, y_{\text{train}})$}
  \end{itemize}
  }
\end{column}
\begin{column}{0.59\textwidth}
  \centering
  \includegraphics[width=\textwidth, height=0.46\textheight, keepaspectratio]{figures/meme.png}
\end{column}
\end{columns}
...
\parbox[t]{0.29\textwidth}{\textbf{1. Pre-training = regularisation}\\Cannot overfit to 1{,}500 rows since it never trains on them via gradient \mbox{descent}}
\hfill
\parbox[t]{0.37\textwidth}{\textbf{2. Noise robustness}\\Bayesian posterior smooths over \mbox{individual flipped labels}}
\hfill
\parbox[t]{0.27\textwidth}{\textbf{3. No hyperparameters}\\Zero risk of overfitting to CV folds}
}
\end{frame}
...
\vspace{0.5em}
\textbf{Method 1: Confident Learning (Cleanlab).} \textit{Probabilistic.}\\
...
\vspace{0.4em}
\textbf{Method 2: Area Under the Margin (AUM, CatBoost).} \textit{Margin-based.}\\
...
\vspace{0.4em}
\textbf{Method 3: Dataset Cartography.} \textit{Training-dynamics.}\\
